...
use crate::{map::{TileMap, TileType}, constants::scale_factor};
...
fn spawn_map(
    ...
    for tile in &tile_map.map {
        match tile.tile_type {
            TileType::WALL=> {
                let start_x = f32::from(tile.x as i16) * (16. * scale_factor) + 8.;
                let start_y = f32::from(tile.y as i16) * (16. * scale_factor) + 8.;
            ...
            if source.index() != inner_source.index() {
                ...
                if let Some(collision) = collision {
                        if (collision == Collision::Right) {
                            println!("source: {}, inner_source: {}", source.index(), inner_source.index());
                        }
